...
from ..models import Promotion, VendorReel, Tag, City, Area, UserPortalConfig
...
class UserPortalConfigModelTest(TestCase):
    ...
    def test_config_ordering(self):
        ...
        configs = UserPortalConfig.objects.all()
        
        # Check actual ordering
        keys = [config.key for config in configs]
        
        # Verify alphabetical ordering
        expected_keys = ['a_config', 'm_config', 'max_search_radius', 'z_config']
        if 'new_setting' in keys:
            expected_keys.insert(-1, 'new_setting')
